...
def format_j_genes(jalignments):
    ...
    if sys.platform == "darwin":
        pr = Popen( [ "../bin/muscle_macOS", "-in", ffile, "-gapopen", "-10", "-out", al_filename, ], stdout=PIPE, stderr=PIPE )
    else:
        pr = Popen( [ "../bin/muscle", "-in", ffile, "-gapopen", "-10", "-out", al_filename, ], stdout=PIPE, stderr=PIPE )
    o, e = pr.communicate()
...
def make_putative_alignments( vsequences, jsequences, calignments = None ):
    ...
    if calignments:
        ...
        output_c_only_stockholm(calignments)
...
def output_C_alignments(alignments, c_name):
    ...
    for species, chain_type in alignments:
        ...
        #print ("%s_%s_%s"%(c_name, translations[species], chain_type))
        #print (alignments[(species, chain_type) ])
    ...
    with open( filename, "w") as outfile:
        ...
        for c_name in all_C_alignments:
            for species, chain_type in all_C_alignments[c_name] :
                ...
                #except:
                #    print (c_name,species, chain_type)
    return filename

def output_c_only_stockholm(all_C_alignments,path=None):
    if path is None:
        path = curated_path

    filename = os.path.join( path, "C_ONLY.stockholm")
    with open( filename, "w") as outfile:
        for c_name in all_C_alignments:
            for species, chain_type in all_C_alignments[c_name] :
                write_stockholm( all_C_alignments[c_name][(species, chain_type) ], "%s_%s_%s"%(translations[species], chain_type,c_name), outfile)
...
def main():
    ...
    all_C_alignments = { "CC":ccalignments,"C1":c1alignments}
    ...
    combined_sequences  = make_putative_alignments( valignments, jalignments ,calignments =all_C_alignments)
